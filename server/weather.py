...
async def get_weather_alerts(state: str) -> str:
    ...
    # Strip NWS test/monitoring messages
    def _is_test(f: dict) -> bool:
        props = f.get("properties", {})
        event = (props.get("event") or "").lower()
        msg_type = (props.get("messageType") or "").lower()
        desc = (props.get("description") or "").lower()
        return (
            "test" in event
            or msg_type == "test"
            or "monitoring message only" in desc
            or "please disregard" in desc
        )

    real_features = [f for f in features if not _is_test(f)]
    if not real_features:
        return "No active alerts for this state (only test/monitoring messages were filtered out)."

    formatted_alerts = [format_alert(feature) for feature in real_features]
...
async def get_latest_news_alerts() -> str:
    ...
    url = f"https://newsapi.org/v2/everything?q=weather+alerts+business&apiKey={news_api_key}"
    data = await make_nws_request(url)
